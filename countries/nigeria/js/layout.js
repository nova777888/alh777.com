...
.contact-float-wrapper { position: fixed; bottom: 140px; right: 20px; display: flex; flex-direction: column; gap: 8px; z-index: 9999; }
...
@media (max-width: 768px) {
  ...
  .contact-float-wrapper { bottom: 136px; right: 12px; }
...
<header class="site-header">
  ...
  // keep TG/WS float buttons above the chat window while it is open
  window.Tawk_API.onChatMaximized = function() {
    var w = document.querySelector('.contact-float-wrapper');
    if (w && window.innerWidth > 768) w.style.bottom = '560px';
  };
